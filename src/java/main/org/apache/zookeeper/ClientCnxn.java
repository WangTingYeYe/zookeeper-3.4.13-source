...
public class ClientCnxn {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    if (!clientCnxnSocket.isConnected()) {
                        // 1.1 不是第一次连接，就随机等待...，重试，随机出时间进行重试
                        if(!isFirstConnect){
                            try {
                        ...
                        // 1.2 建立连接，socket建立成功后会调用SendThread.primeConnection()
                    ...
                    // 3、如果连接失败，看是否超过了connectTimeout，超过了则抛异常SessionTimeoutException，但这个异常会被上层catch住，尝试重连
                    if (to <= 0) {
                        String warnInfo;
    ...
    public ReplyHeader submitRequest(RequestHeader h, Record request,
        ...
        //在这里等待  锁是 当前发送的 数据包。 那么必定有 线程来唤醒它，
        //其实 就是sendThread 读取到服务端返回的数据是。把数据填充完成后，唤醒。
        synchronized (packet) {
    ...
    /**
     * 所有的 消息 都放进outgoingQueue这个队列里。然后 sendThread 线程会 一直遍历来这里取
     * @param h
     * @param r
     * @param request
     * @param response
